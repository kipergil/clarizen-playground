...
namespace BBHPro.Playground.Helpers
{
    public static class AlwaysConvert
    {
        ...
        public static int[] ToIntArray(string parseValue)
        {
            ...
            {
                result[index] = ToInt32(token.Trim());
                index++;
            }
            return result;
        }
        ...
        public static string ToList(string delimiter, int[] intArray)
        {
            if ((intArray == null) || (intArray.Length == 0)) return string.Empty;
        ...
        /// </summary>
        /// <param name="delimiter">The delimiter character for the list, e.g. comma.</param>
        /// <param name="intList">The list of int to convert to comma delimited list</param>
        /// <returns>A delimited string list containing the values of the int list</returns>
        public static string ToList(string delimiter, List<int> intList)
        {
            if (intList == null || intList.Count == 0) return string.Empty;
            return ToList(delimiter, intList.ToArray());
        }
